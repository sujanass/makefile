...
calc_func(ee_s16 *pdata, core_results *res)
{
   ...
   // ee_printf("Insi list cal fun:\n");
    ...
    //ee_printf("list 69: data:%x ,optype:  %x\n",data,optype);
   ...
   // ee_printf("list 79: data:%x ,flag:  %x\n",data,flag);
        switch (flag)
        {
            case 0:
   ...
   // ee_printf("list 86 swith : dtype :%x ,flag:  %x\n",dtype ,flag);
   ...
   // ee_printf("list 96 swith0 : retval:   %x\n", retval);
                break;
            case 1:
  ...
  //  ee_printf("list 103 swith1 : retval:   %x\n", retval);
                   break;
            default:
                retval = data;
                break;
     ...
     //ee_printf("list 112 retval %x *pdata %x\n",retval ,*pdata) ;
       return retval;
    }
}
...
core_bench_list(core_results *res, ee_s16 finder_idx)
{
   ...
   //  ee_printf("list 171 find_num %x info.idx  %x\n",find_num ,info.idx ) ;
    ...
    // ee_printf("list 181  info.data16  %x this_find%xlist %x \n", info.data16,this_find ,list) ;
        if (this_find == NULL)
        {
            missed++;
            retval += (list->next->info->data16 >> 8) & 1;
   // ee_printf("list 185 (list->next->info->data16 >> 8) & 1; %x\n",((list->next->info->data16 >> 8) & 1)) ;
        }
        else
        {
            found++;
   ...
   // ee_printf("list 193 (this_find->info->data16 >> 9)  %x\n",((this_find->info->data16 >> 9))) ;
    // ee_printf("list 192 retval %x found %x\n",retval , found) ;
...
core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
   ...
   // ee_printf("init list 266 per_item %x size %x *memblock_end %x memblock %x  *datablock %x memblock_end %x datablock %x  \n",per_item , size,*memblock_end,memblock,*datablock,memblock_end,datablock);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        core_list_insert_new(
            list, &info, &memblock, &datablock, memblock_end, datablock_end);
   // ee_printf("init list 294 info.data16 %x \n",info.data16 );
    }
...
core_list_find(list_head *list, list_data *info)
{
    ...
    if (info->idx >= 0)
    {
        //z_printf("inside if before while list %d\n",list);
        while (list && (list->info->idx != info->idx))
            list = list->next;
        //z_printf("inside if after while list %d\n",list);
        return list;
    }
    else
    {
       // z_printf("inside else before while list %d\n",list);
        while (list && ((list->info->data16 & 0xff) != info->data16))
            list = list->next;
      //  z_printf("inside else after while list %d\n",list);
        return list;
    }
}
...
{
       // z_printf("inside list reverse list %d\n",list);
    list_head *next = NULL, *tmp;
    while (list)
    {
        tmp        = list->next;

       // z_printf("inside list reverse inside while list->next %d\n",tmp);
        list->next = next;
       // z_printf("inside list reverse inside while  next %d\n",next);
        next       = list;
        list       = tmp;

       // z_printf("inside list reverse in while list %d\n",list);
    }
    return next;
}
    ...
    while (1)
    {
        ...
        while (p)
        {
            ...
            while (psize > 0 || (qsize > 0 && q))
            {
                ...
                if (psize == 0)
                {
          // z_printf("merge list psize if q is %x\n",q);
                ...
                {
          // z_printf("merge list qsize is %x , p is %x, q is %x\n",qsize,p,q);
                ...
                {
         //  z_printf("merge list cmp is p->info: %x q->info:  %x\n",p->info, q->info);
                ...
                {
         //  z_printf("merge list else is q%x qsize%x\n",q,qsize);
        ...
        if (nmerges <= 1) /* allow for nmerges==0, the empty list case */
         //  z_printf("one merge list is %x\n",list);
            return list;

        /* Otherwise repeat, merging lists twice the size */
        insize *= 2;

         //  z_printf("merge list is %x insize is %x\n",list,insize);
    }
