...
{
    ee_printf("inside matrix init\n");
    ee_u32  N = 0;
    MATDAT *A;
    MATDAT *B;
    ee_s32  order = 1;
    MATDAT  val;
   // ee_printf("matrix init 172 N %x *A %x *B %x val %x \n",N,*A,*B,val);
    ee_u32  i = 0, j = 0;
    if (seed == 0)
        seed = 1;
    while (j < blksize)
    ...
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
            ...
            A[i * N + j] = val;
            order++;
  //  ee_printf("matrix init 197 N %x *A %x *B %x val %x \n",N,*A,*B,val);
...
matrix_sum(ee_u32 N, MATRES *C, MATDAT clipval)
{
    ...
    for (i = 0; i < N; i++)
    {
        for (j = 0; j < N; j++)
        {
            ...
            prev = cur;
   // ee_printf("matrix sum 243 N %x C %x cur %x tmp %x \n",N,C,cur,tmp);
