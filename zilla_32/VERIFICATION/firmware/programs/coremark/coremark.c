...
{
#endif
        ee_printf("inside main\n");
    ee_u16       i, j = 0, num_algorithms = 0;
    ee_s16       known_id = -1, total_errors = 0;
    ee_u16       seedcrc = 0;
    CORE_TICKS   total_time,total_cycle,total_instn;
...
#error "Please define a way to initialize a memory block."
    ...
    for (i = 0; i < MULTITHREAD; i++)
    {
        ...
        ee_printf("exit initialisations\n");
    }

        ee_printf("outside initialisations\n");
    /* automatically determine number of iterations if not set */
    if (results[0].iterations == 0)
    {
        ee_printf("inside 0 iterations");
...
//ee_printf("starting timer\n");
   ...
   // ee_printf("starting execution\n");
...
#endif
    stop_time();
    ee_printf("stop timer");
    total_time = get_time();
    total_cycle=stop_cycle;
    total_instn=stop_instn;
     //total_cycle=get_cycle();
    //total_instn=get_instn();
    ...
    /* and report results */
    ee_printf("CoreMark Size    : %l\n", (long unsigned)results[0].size);
    ee_printf("Total ticks      : %l\n", (long unsigned)total_time);

    ee_printf("Total cycle     : %l\n", (long unsigned)total_cycle);
    ee_printf("Total instn      : %l\n", (long unsigned)total_instn);
#if HAS_FLOAT
    ...
    if (time_in_secs(total_time) > 0)
        ee_printf("Iterations/Sec   : %f\n",
                  default_num_contexts * results[0].iterations
                      / time_in_secs(total_time));
#else
          if((total_time>100000) & (total_time<1000000))
{
    ee_printf("Total time (secs):0.00%d\n", time_in_secs(total_time));
}
    else if((total_time>1000000) & (total_time<10000000))
{
    ee_printf("Total time (secs):0.0%d\n", time_in_secs(total_time));
    
}
    if (time_in_secs(total_time) > 0)
        ee_printf("Iterations/Sec   : %d.%d\n",
                  (default_num_contexts * results[0].iterations * CLOCKS_PER_SEC
                      / time_in_secs(total_time))/100, (default_num_contexts * results[0].iterations * CLOCKS_PER_SEC
                      / time_in_secs(total_time))%100);
#endif
